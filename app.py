...
with t1:
    if not df_p.empty:
        ...
        # --- PREDICCIÓN BOMBA INDIVIDUAL POR ROL (CASA/FUERA) ---
        if not df_fin.empty:
            st.divider()
            ...
            if len(h_l_home) > 0 and len(h_v_away) > 0:
                ...
                <div style="background-color: #ff4b4b; padding: 25px; border-radius: 15px; border-left: 12px solid #8B0000; color: white; text-align: center;">
                    ...
                    <p style="font-size: 1.1rem; line-height: 1.6; margin-top: 15px;">
                        ...
                        Por otro lado, el equipo visitante <b>{vis}</b> ha marcado al menos 1 gol en {m1_v} de {t_v} partidos como visitante y de esos {m1_v} ha marcado más de 1.5 goles, 
...
with t2:
    ...
    if not df_h.empty:
        # --- FILTROS DE HISTORIAL RESTAURADOS ---
        h1, h2 = st.columns(2)
        ...
        # --- DEFINICIÓN DE df_res (EL FIX DEL NAMEERROR) ---
        ...
        st.dataframe(df_res[['Fecha', 'Jornada', 'Liga', 'Equipo Local', 'Equipo Visitante', 'Marcador', 'Doble Oportunidad', 'Over 1.5', 'Over 2.5', 'BTTS']]
                     .style.applymap(color_letras_historial, subset=['Doble Oportunidad', 'Over 1.5', 'Over 2.5', 'BTTS']), 
