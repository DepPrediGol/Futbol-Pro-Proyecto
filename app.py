    ...
    if not df_p.empty:
        ...
        if not df_fin.empty:
            ...
            if not h_l.empty and not h_v.empty:
                t_l, m1_l, m2_l, w_l = len(h_l), (h_l['G_L']>=1).sum(), (h_l['G_L']>=2).sum(), (h_l['1X'].str.contains('✅')).sum()
                t_v, m1_v, m2_v, w_v = len(h_v), (h_v['G_V']>=1).sum(), (h_v['G_V']>=2).sum(), (h_v['X2'].str.contains('✅')).sum()
