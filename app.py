...
def ventana_analisis(r, df_h):
    ...
    for eq, col_rol, nombre_rol in roles:
        ...
        if not df_eq.empty:
            ...
            m_1x, m_o15, m_btts = (df_eq['Double chance'].str.contains('✅').sum() / len(df_eq)), (df_eq['Over 1.5'].str.contains('✅').sum() / len(df_eq)), (df_eq['Btts'].str.contains('✅').sum() / len(df_eq))
...
with t1:
    if not df_p.empty:
            ...
            for i, (m, tit) in enumerate(mks):
                with cols[i]:
                    ...
                    for idx, r in top.iterrows():
                        ...
                        if st.button(f"{r['Date']} {r['Time']}\n{r['League']}\n{r['Match']}\n⭐ {etq}: {r[m]:.0%}", key=f"t4_{m}_{idx}"): ventana_analisis(r, df_h)
        st.divider()
        ...
        if not df_fin.empty:
            ...
            h_l_home, h_v_away = df_h[(df_h['Home team'] == d_top['Home team']) & (df_h['League'] == d_top['League'])], df_h[(df_h['Away team'] == d_top['Away team']) & (df_h['League'] == d_top['League'])]
            if not h_l_home.empty and not h_v_away.empty:
                ...
                st.markdown(f"""<div style="background-color: #ff4b4b; padding: 25px; border-radius: 15px; border-left: 12px solid #8B0000; color: white; text-align: center;"><h2 style="color: white !important; margin: 0;">💣 PREDICCIÓN BOMBA DETECTADA 💣</h2><p style="font-size: 1.1rem; margin-top: 15px;">El equipo local <b>{d_top['Home team']}</b> lleva {m1_l}/{t_l} marcando y ha ganado/empatado {w1x_l}/{t_l}. El visitante <b>{d_top['Away team']}</b> ha marcado en {m1_v}/{t_v} y ganado/empatado {wx2_v}/{t_v}.</p><div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; margin-top: 20px;"><div style="background: white; color: #ff4b4b; padding: 10px; border-radius: 10px;">🛡️ <b>{("1X" if d_top['1X']>=d_top['X2'] else "X2")}: {max(d_top['1X'],d_top['X2']):.0%}</b></div><div style="background: white; color: #ff4b4b; padding: 10px; border-radius: 10px;">🥅 <b>Over 1.5: {d_top['Over 1.5']:.0%}</b></div><div style="background: white; color: #ff4b4b; padding: 10px; border-radius: 10px;">⚽ <b>Over 2.5: {d_top['Over 2.5']:.0%}</b></div><div style="background: white; color: #ff4b4b; padding: 10px; border-radius: 10px;">🤝 <b>Btts: {d_top['Btts']:.0%}</b></div></div></div>""", unsafe_allow_html=True)
        
        # --- NUEVA UBICACIÓN DEL HISTORIAL (ABAJO DE LA BOMBA) ---
        st.divider()
        st.markdown("## 📜 HISTORIAL DE RESULTADOS")
        if not df_h.empty:
            hc1, hc2 = st.columns(2)
            with hc1: slh = st.selectbox("Filtrar Liga Historial:", ["TODAS"] + lgs, key="h_l_tab1")
            with hc2:
                df_hh = df_h if slh=="TODAS" else df_h[df_h['League']==slh]
                sjh = st.selectbox("Filtrar Jornada Historial:", ["TODAS"] + sorted(df_hh['Matchday'].unique().tolist(), reverse=True) if not df_hh.empty else ["TODAS"], key="h_j_tab1")
...
with t2:
    ...
    st.info("Esta sección está siendo preparada para el análisis de ligas mundiales de baloncesto (NBA, EuroLeague, etc.). Próximamente.")