        ...
        try:
            ...
            for _, f in df.iterrows():
                ...
                if g:
                    ...
                    historicos.append({'Fecha': f['Fecha'], 'Liga': ln, 'Jornada': f['Jornada'], 'Equipo Local': f['Equipo Local'], 'Equipo Visitante': f['Equipo Visitante'], 'Marcador': f"{g[0]} - {g[1]}", 'G_L': g[0], 'G_V': g[1], 'Doble Oportunidad': f"{pk} {'✅' if (g[0]>=g[1] if pk=='1X' else g[1]>=g[0]) else '❌'} ({max(p1x,px2):.0%})", 'Over 1.5': f"{'✅' if (g[0]+g[1])>1.5 else '❌'} ({po15:.0%})", 'Over 2.5': f"{'✅' if (g[0]+g[1])>2.5 else '❌'} ({po25:.0%})", 'BTTS': f"{'✅' if (g[0]>0 and g[1]>0) else '❌'} ({pb:.0%})"})
                else:
                    actuales.append({'Fecha': f['Fecha'], 'Fecha_dt': f['Fecha_dt'], 'Jornada': f['Jornada'], 'Liga': ln, 'Local': f['Equipo Local'], 'Visitante': f['Equipo Visitante'], 'Partido': f"{f['Equipo Local']} vs {f['Equipo Visitante']}", '1X': pl+pe, 'X2': pv+pe, 'Over 1.5': po15, 'Over 2.5': po25, 'BTTS': pb})
        except: continue
...
with t1:
    if not df_p.empty:
        ...
        if len(fechas) > 0:
            ...
            mks, cols = [('1X', '🛡️ Doble Oportunidad'), ('Over 1.5', '🥅 Over 1.5'), ('Over 2.5', '⚽ Over 2.5'), ('BTTS', '🤝 BTTS')], st.columns(4)
            for i, (m, tit) in enumerate(mks):
                with cols[i]:
                    st.markdown(f"#### {tit}")
                    top = df_t4.nlargest(4, m)
                    for idx, r in top.iterrows():
                        if st.button(f"{r['Fecha']}\n{r['Liga']}\n{r['Partido']}\n⭐ Prob: {r[m]:.0%}", key=f"t4_{m}_{idx}"): ventana_analisis(r, df_h)
        st.divider()
        ...
        if not df_fin.empty:
            ...
            if len(h_l_home) > 0 and len(h_v_away) > 0:
                ...
                m1_l, m2_l, r1_l, w1x_l = (h_l_home['G_L'] >= 1).sum(), (h_l_home['G_L'] >= 2).sum(), (h_l_home['G_V'] >= 1).sum(), (h_l_home['G_L'] >= h_l_home['G_V']).sum()
                ...
                st.markdown(f"""<div style="background-color: #ff4b4b; padding: 25px; border-radius: 15px; border-left: 12px solid #8B0000; color: white; text-align: center;"><h2 style="color: white !important; margin: 0;">💣 PREDICCIÓN BOMBA DETECTADA 💣</h2><p style="font-size: 1.1rem; line-height: 1.6; margin-top: 15px;">El equipo local <b>{loc}</b> lleva {m1_l} de {t_l} marcando al menos 1 gol en casa y ha ganado o empatado en {w1x_l} de {t_l}. Por otro lado, <b>{vis}</b> ha marcado al menos 1 gol en {m1_v} de {t_v} partidos de visitante.</p><div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; margin-top: 20px;"><div style="background: white; color: #ff4b4b; padding: 10px; border-radius: 10px;">🛡️ <b>{etiqueta_texto}</b></div><div style="background: white; color: #ff4b4b; padding: 10px; border-radius: 10px;">🥅 <b>Over 1.5:</b> {d_top['Over 1.5']:.0%}</div><div style="background: white; color: #ff4b4b; padding: 10px; border-radius: 10px;">⚽ <b>Over 2.5:</b> {d_top['Over 2.5']:.0%}</div><div style="background: white; color: #ff4b4b; padding: 10px; border-radius: 10px;">🤝 <b>Ambos Marcan:</b> {d_top['BTTS']:.0%}</div></div></div>""", unsafe_allow_html=True)
        else: st.info("No hay predicciones futuras para esta liga.")
...
with t2:
    ...
    if not df_h.empty:
        ...
        else: st.info("No hay historial disponible para esta selección.")